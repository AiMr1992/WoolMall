...
public class ScrollerNumberPicker extends View {
	...
	/** 设置数据 */
	private List<CityInfoEntity> dataList = new ArrayList<CityInfoEntity>();
	/** 按下的坐标 */
	private int downY;
	/** 按下的时间 */
	...
	private void initData() {
		...
		for (int i = 0; i < dataList.size(); i++) {
			...
			itmItemObject.id = i;
			itmItemObject.itemText = dataList.get(i).getRegion_name();
			itmItemObject.x = 0;
	 ...
	 * @param stringList
	 */
	public void initData(List<String> stringList){
		if(stringList != null) {
			isClearing = true;
			itemList.clear();
			for (int i = 0; i < stringList.size(); i++){
				ItemObject itmItemObject = new ItemObject();
				itmItemObject.id = i;
				itmItemObject.itemText = stringList.get(i);
				itmItemObject.x = 0;
				itmItemObject.y = i * unitHeight;
				itemList.add(itmItemObject);
			}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		selectedColor = attribute.getColor(
				R.styleable.NumberPicker_selectedTextColor, 0xffff0000);
		lineColor = attribute.getColor(R.styleable.NumberPicker_lineColor,
				0xff000000);
		maskHight = attribute.getDimension(R.styleable.NumberPicker_maskHeight,
				48.0f);
	 ...
	 * @param data
	 */
	public void setData(List<CityInfoEntity> data) {
		this.dataList = data;
		initData();
	}
	...
	private class ItemObject {
		...
		public void drawSelf(Canvas canvas) {
			...
			canvas.drawText(itemText, x + controlWidth / 2 - textRect.width()
							/ 2, y + move + unitHeight / 2 + textRect.height() / 2-5,
					textPaint);
		}
